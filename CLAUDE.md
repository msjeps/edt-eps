...
25. ✅ Refonte UI/UX confort + accessibilité (12/06/2026) : audit complet (`AUDIT_UI_UX.md`) puis implémentation. **Typo** relevée (base 16px), **containers** marqués (`.card` bordure+ombre), **page Exports uniformisée** (`.export-card`/`.export-card-head`). **8 quick wins a11y** : `:focus-visible` global + `.sr-only`, `prefers-reduced-motion` + fin du clignotement des conflits, contraste `--c-text-muted` AA, bug `form-control`→`form-input`, `<h2>` redondant retiré, `aria-label`/`aria-hidden`, empty-state Exports, `.dashboard-card` alignée. **Design system** : tokens sémantiques `*-text/*-border`, classes `.section-title`/`.callout`/`.badge-new`/`.validation-list`, Dashboard+Exports migrés, cartes cliquables → `<button>`. **Confort EDT** : blocs 44px sur 2 lignes + **toggle densité Confort/Compact** (mémorisé). **Cibles** agrandies (btn-sm 34/btn-xs 30/btn-icon-sm 32). **Sélecteur de période global** dans le header (`src/utils/period-store.js`), synchronisé EDT↔Vues et mémorisé — `styles/main.css`, `styles/grid.css`, `styles/components.css`, `src/app.js`, `src/views/{edt/grid,vues/vues,exports/exports,dashboard}.js`, `index.html`
26. ✅ Daltonisme installations — second canal texture (12/06/2026) : toggle « Motifs » dans la toolbar EDT (mémorisé `localStorage['edt-patterns']`, off par défaut) qui applique 9 textures neutres distinctes aux blocs **et** à la légende. Affectation **par index d'installation** (`data-pattern="0..8"` calculé dans `renderEdt`, indépendante de la palette codée en dur → marche pour tout jeu d'installations) ; la légende devient la clé (pastille blanche 16px + texture + liseré couleur). Encre `rgba(15,23,42,…)` sous le texte, lisibilité préservée — `src/views/edt/grid.js`, `styles/grid.css`

27. ✅ Campagne anti-inline — passe donnees/wizard/programmation (12/06/2026) : tokenisation complète des couleurs/tailles hardcodées dans les `style=` (hex → `var(--c-*)`, rem → `var(--fs-*)`/`var(--sp-*)` ; **0 couleur hardcodée résiduelle**, débloque le mode sombre) + couche utilitaire `u-*` (12 classes dans `styles/main.css`) avec migration des patterns répétés sur éléments sans `class=` → **252 → 191 attributs `style=`** (−24 %). Reste = dynamique data-driven légitime + patterns sur éléments déjà classés — `src/views/{donnees/donnees,wizard/wizard,programmation/programmation}.js`, `styles/main.css`
28. ✅ Mode sombre / confort (12/06/2026) : bouton bascule lune/soleil dans le header, état géré par `src/utils/theme-store.js` (`data-theme` sur `<html>`, mémorisé `localStorage['edteps-theme']`, **défaut = `prefers-color-scheme`**), script inline anti-FOUC dans `index.html`. Bloc `[data-theme="dark"]` dans `main.css` redéfinissant **tous** les tokens (surfaces, textes, bordures, fonds sémantiques, ombres, sidebar) + `color-scheme`. Surcharges ciblées : grille + blocs EDT + encre des motifs daltonisme (`grid.css`), tokenisation des couleurs sémantiques hardcodées de `components.css` (41) et `programmation.css` (15), `.tag-*` (main.css), onglet actif Vues, fond formulaire Exports. Vérifié au navigateur : 8 vues + EDT + modales basculent sans fond clair ni texte sombre résiduel, thème clair inchangé — `src/utils/theme-store.js`, `src/app.js`, `index.html`, `styles/{main,grid,components,programmation}.css`, `src/views/{exports/exports,vues/vues}.js`

29. ✅ Import lieux/installations (15/06/2026) : bouton **📥 Importer lieux** dans Données > Installations (entre "Import mairie" et "+ Lieu"). Modale avec deux options : **JSON** (format structuré exporté depuis l'app) ou **CSV** (2 colonnes `LIEUX` et `INSTALLATIONS`, parsé via Papa Parse, détection automatique des en-têtes insensible à la casse). Import intelligent : doublons ignorés par nom, installations nouvelles ajoutées aux lieux existants, Ctrl+Z supporté. Fichier de référence `data/ref/installations-antibes.json` (13 lieux, 56 installations Direction des Sports Antibes) **non exposé dans l'UI** (confidentiel, à envoyer directement aux collègues) — `src/views/donnees/donnees.js`, `data/ref/installations-antibes.json`

### Reste à implémenter
- MVP Phase 1 complet. Reste des chantiers UI/UX de fond (cf. `AUDIT_UI_UX.md` §10) : états champs (hover + aria-busy), audit a11y automatisé. Anti-inline : poursuite possible sur les patterns à fusion de classes (faible valeur). Daltonisme : fait sur l'EDT ; reste éventuellement les vues « par installation ». Mode sombre : fait ; polish possible sur quelques bleus de la modale Versions (sans token exact).
